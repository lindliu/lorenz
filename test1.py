...
class LatentSDE(nn.Module):
    ...
    #     z0 = xs[0,:,:]
    #     zs = torchsde.sdeint(self, z0, ts, dt=1e-2, method=method)
    #     return zs
    ...
    def sample(self, batch_size, ts, bm=None):
        ...
        z0 = torch.randn(size=(batch_size, *self.pz0_mean.shape[1:]), device=self.pz0_mean.device)*STD
...
def make_dataset(t0, t1, batch_size, noise_std, train_dir, device):
    ...
    if False:#os.path.exists(data_path):
        ...
        _y0 = torch.randn(batch_size, 3, device=device)*STD
...
def main(
    ...
    for global_step in tqdm.tqdm(range(1, num_iters + 1)):
        ...
        # pxs = latent_sde(xs, ts, noise_std, adjoint, method)
        # loss = mse(pxs, xs)
...
if __name__ == "__main__":
    ...
    _y0 = torch.randn(batch_size, 3, device=device)*STD
    ts = torch.linspace(t0, t1, steps=100, device=device)
    xs = torchsde.sdeint(StochasticLorenz(),_y0, ts)
    fig = plt.figure(figsize=(20, 9))
    gs = gridspec.GridSpec(1, 2)
    ax00 = fig.add_subplot(gs[0, 0], projection='3d')
    ax01 = fig.add_subplot(gs[0, 1], projection='3d')
